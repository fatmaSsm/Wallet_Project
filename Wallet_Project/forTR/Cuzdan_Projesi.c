...
void verileriKaydet() {
    ...
    for (int i = 0; i < kullaniciSayisi; i++) {
        fprintf(fp, "%s;%s;%s;%d;%d;", adlar[i], soyadlar[i], sifreler[i],
...
void verileriYukle() {
    ...
    while (fgets(satir, sizeof(satir), fp)) {
        ...
        char ad[MAX_LEN], soyad[MAX_LEN], sifre[MAX_LEN];
        int gSay=0, dSay=0;
        char gelirStr[512], giderStr[512];
        float b = 0.0f;

        // Satırı ayır ve oku
        int n = sscanf(satir, "%49[^;];%49[^;];%49[^;];%d;%d;%511[^;];%511[^;];%f",
                       ad, soyad, sifre, &gSay, &dSay, gelirStr, giderStr, &b);
        if(n < 8) continue; // bozuk satırı atla

        // Matrislere kaydet
        strncpy(adlar[kullaniciSayisi], ad, MAX_LEN-1); adlar[kullaniciSayisi][MAX_LEN-1]='\0';
        strncpy(soyadlar[kullaniciSayisi], soyad, MAX_LEN-1); soyadlar[kullaniciSayisi][MAX_LEN-1]='\0';
        strncpy(sifreler[kullaniciSayisi], sifre, MAX_LEN-1); sifreler[kullaniciSayisi][MAX_LEN-1]='\0';

        gelirSay[kullaniciSayisi] = gSay;
        giderSay[kullaniciSayisi] = dSay;

        // Gelirleri ayır
        for(int i=0;i<gSay;i++) {
            char *ptr = i==0 ? gelirStr : strchr(gelirStr, ',')+1;
            if(ptr) sscanf(ptr, "%f", &gelirler[kullaniciSayisi][i]);
        }

        // Giderleri ayır
        for(int i=0;i<dSay;i++) {
            char *ptr = i==0 ? giderStr : strchr(giderStr, ',')+1;
            if(ptr) sscanf(ptr, "%f", &giderler[kullaniciSayisi][i]);
        }

        // Bakiye
        bakiye[kullaniciSayisi] = b;
